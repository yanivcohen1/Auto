...
namespace com.who.tests.clients
{
    ...
    public class EurograndEn : Automation_Lib
    {
        ...
        ///The main test
        ///</summary>
        public void EurograndEnClientTestDebug()
        {
            ctx = Commons.InitSpring("EurograndClientEnContext.xml");
            ...
            WriteToPlayersUsersfile(userName + ", " + "EurograndClientEn");
            PopupMsgPage popupMsgPage = (PopupMsgPage)ctx.GetObject(typeof(PopupMsgPage).Name);
            CashierPage cashierPage;
            try
            {
                cashierPage = popupMsgPage.clickCloseMsgEuro();//CloseSuccessRegisterPopup
            }
            catch (Exception)
            {
                cashierPage = (CashierPage)ctx.GetObject(typeof(CashierPage).Name);
            }
