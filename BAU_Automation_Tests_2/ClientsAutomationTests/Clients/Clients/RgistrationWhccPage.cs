...
namespace com.who.tests.clients
{
	...
	/// </summary>
	public class RegistrationWhccPage : Automation_Lib
	{
		...
		protected string ContinueRegister { get; set; }
		...
		public PopupMsgPage RegisterDataFill(string userName)
		{
			...
			WriteText(editControls[3], City);//email
			WriteText(editControls[4], Address);//Address
			WriteText(editControls[5], PostalCode);//city
			WriteText(editControls[7], MobileNumber);//postal code
			WriteText(editControls[8], userName);//mobile number
			...
			WriteText(editControls[9], Password);//user name
		...
		/// </summary>
		/// <returns></returns>
		public GameModePage RegisterDataFillNew(string userName)
		{
			//MessageBox.Show("Data fill new");
			Condition controlNameCondition = new PropertyCondition(AutomationElement.NameProperty, CreateAccount);
			...
			for (int i = 0; i < 20; i++)
			{
				try
				{
					...
					//AutomationElement control = mainWindow.FindFirst(TreeScope.Element | TreeScope.Descendants | TreeScope.Children, controlNameCondition);
					//Console.WriteLine(controls.Count);
					setforgrandwindow(mainWindow);
					Thread.Sleep(1000);
					ClickElement(mainWindow);
					//ClickElement(FindControlById(checkControls[1]));//I agree
					p = TrayClick(control);
					break;
				}
				catch (Exception)
			...
			AutomationElementCollection comboControls = mainWindow.FindAll(TreeScope.Element | TreeScope.Descendants, comboBoxCondition);
			//Thread.Sleep(5000);
			//MessageBox.Show("Before click continue");
			Condition controlNameCondition2 = new PropertyCondition(AutomationElement.NameProperty, ContinueRegister);
			//AutomationElement mainWindow2 = waitforWindowNameClass(WinClass, controlNameCondition2, 60, false);
			AutomationElementCollection checkControls2 = null;
			System.Windows.Point p2;
			for (int i = 0; i < 15; i++)
			{
				checkControls2 = mainWindow.FindAll(TreeScope.Element | TreeScope.Descendants, controlNameCondition2);
				if (checkControls2.Count != 0)
				{
					try
					{
						//p2 = TrayClick(checkControls2[0]);
						ClickElement(checkControls2[0]);
						break;
					}
					catch (Exception){ }					
				}
			...
			//ClickElement(checkControls2[0]);
			//MessageBox.Show("After click continue");
			//setforgrandwindow(mainWindow);
			Condition editCondition = new PropertyCondition(AutomationElement.LocalizedControlTypeProperty, "edit", PropertyConditionFlags.IgnoreCase);
			AutomationElementCollection editControls = mainWindow.FindAll(TreeScope.Element | TreeScope.Descendants, editCondition);
			WriteText(editControls[0], FirstName);//first name
			...
			checkBoxCondition = new PropertyCondition(AutomationElement.LocalizedControlTypeProperty, "check box", PropertyConditionFlags.IgnoreCase);
			checkControls = mainWindow.FindAll(TreeScope.Element | TreeScope.Descendants, checkBoxCondition);
			ClickElement(checkControls[1]);//I agree
