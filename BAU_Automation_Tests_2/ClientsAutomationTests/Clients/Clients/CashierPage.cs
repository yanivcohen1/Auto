...
namespace com.who.tests.clients
{
    public class CashierPage : Automation_Lib
    {
        ...
        public void ClickOnCashier()
        {
            ...
            Thread.Sleep(5000);
            Condition controlNameCondition = new PropertyCondition(AutomationElement.LocalizedControlTypeProperty, "window");
        ...
        /// Enter Cashier form Details
        /// </summary>
        public void EnterDetailsChrome()
        {
            //Cashier form
            Thread.Sleep(15000);
            ...
            SendString("{Tab}");
            SendString("v");//visa delta
            SendString("{Tab}");
            SendString(CardNumber);//card num
            SendString("{Tab}");
            //SendString(CardNumber);//card num
            SendString("{Tab}");
            //SendString(CardNumber);//card num
            SendString("{Tab}");
            SendString("{DOWN}");//month 01
            SendString("{Tab}");
            SendString("{DOWN}");//year 2025
            SendString("{Tab}");
            SendString("{Tab}");
            SendString(CVV2);
            SendString("{Tab}");
        ...
        /// successfully registered message
        /// </summary>
        public DepositPage CloseSuccessRegisterPopupChrome()
        {
            Thread.Sleep(5000);
            PropertyCondition nameCondition = new PropertyCondition(AutomationElement.ClassNameProperty, "Chrome_RenderWidgetHostHWND");
            mainWindow = waitforWindowNameClass("PTIODEVICE", nameCondition, 30, false);
