...
public class MyEventListener implements WebDriverEventListener {
		...
		//System.out.println(LastError);
		 if (!LastError.contentEquals(findBy) || (System.currentTimeMillis()-lastTime)>5000) {
			 ...
			 if (error.getClass().equals(NoSuchElementException.class)){
				 ...
				 MyLogger.ERROR("WebDriver error: Element not found "+findBy);
				 //logger.error("WebDriver error: Element not found "+findBy);
	        } else {
	         ...
	         //org.testng.Reporter.log("This is only test" + Utility.newlineHtml, 0, false);
	        	MyLogger.ERROR("WebDriver error: with Element "+ findBy);
	        	//logger.error("WebDriver error: Element not found "+findBy);
	        }
		}
	...
	public void beforeChangeValueOf(WebElement element, WebDriver driver) {
		Reporter.log("About to write to " + element.getTagName() + " find"+ findBy, true);
	}

	@Override
	public void beforeClickOn(WebElement element, WebDriver driver) {
		Reporter.log("About to click on " + element.getTagName() + " find"+ findBy, true);
		//logger.info("About to click on " + element.getTagName() + " find"+ findBy);
	}
	...
	public void beforeNavigateTo(String url, WebDriver driver) {
		Reporter.log("About to navigating to:'"+url+"'", true);
		//System.out.println("About to navigating to:'"+url+"'" + Utility.newlineHtml);
	}

	@Override
	public void beforeScript(String script, WebDriver driver) {
		Reporter.log("About to execute script:'"+script+"'", true);
	}
