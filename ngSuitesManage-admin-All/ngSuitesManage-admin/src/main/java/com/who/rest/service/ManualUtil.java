...
public class ManualUtil extends Common{
	...
	@Transactional
	public List<String> getAllManualTets() throws IllegalArgumentException {
		List<Tests> tests = null;
		SuitesData returnList = new SuitesData();
		try {// Manual Tests
		...
		return returnList.getTestsName();
	}

	@Transactional
	public String RunManualTest(SuitesData testData, String testManualContent) throws IllegalArgumentException {
			if (testData.getTestName() != null) {
				...
				try {
					return RunJenkins.runTests(testData).get(0);
				} catch (Exception e) {
					e.printStackTrace();
