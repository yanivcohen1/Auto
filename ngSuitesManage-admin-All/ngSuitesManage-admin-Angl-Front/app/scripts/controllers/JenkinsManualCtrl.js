'use strict';
angular.module('ngSuitesManageAdminYeomanApp').controller('ManualCtrl',['$scope','$log', '$window', '$http',
                                                               function ($scope, $log, 	  $window,   $http) {
	var manualUrl = AppConfig.apiServiceBaseUri() + '/rest/jenkinsManualTests/';
    $scope.options = [];
	$scope.content = "";
	$scope.selected;
    ...
    function getmanualTests() {
    	$http.get(manualUrl + 'urlTest').success(function(allmanualTests) {
    ...
    $scope.runTest = function(content){
    	...
    	$http.post(manualUrl+ 'urlTest',{testName:$scope.selected, testContent:$scope.content}).success(function(jenkinsUrl) {
