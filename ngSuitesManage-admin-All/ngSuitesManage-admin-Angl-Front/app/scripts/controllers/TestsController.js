...
app.controller('TestsController', function($scope, $filter, $modal, $log, testsService, $stateParams, ngTableParams) {
	
    $scope.data = [];
    $scope.suiteId = parseInt($stateParams.suiteID);
    var tests;
    readTests();
    
    function readTests() {
        testsService.readTests($scope.suiteId ).then(function(allSutiesFactory) { // success
    ...
    $scope.updateTest = function (suiteId, testId, suiteName, classesIDs, parametersIDs) {
        ...
        testsService.updateTest(suiteId, suiteName, testId, testName,
                classesNamef, classesIDs, parametersNamef, parametersIDs, parametersValuef).then(function(resultSize) 
        	{// success
	        	$scope.setEditId(-1);
	        	readTests();
...
app.controller('newTestModalCtrl', function ($scope, $log, testsService, $modalInstance, items) {
	  ...
	  $scope.insertTest = function (suiteName, testName, classesNamef, parametersNamef, parametersValuef) {
	        //alert(TestName);
	        testsService.newTest(suiteName, testName, classesNamef, parametersNamef, parametersValuef ).then(function(resultSize) 
	         {// success
	        	$modalInstance.close($scope.selected.item);
	    	 }, function(errResponse) {// fail
	    		//alert("Server Error");
