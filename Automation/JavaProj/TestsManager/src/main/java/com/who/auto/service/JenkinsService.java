...
public class JenkinsService {
    ...
    public String runTests(List<RunTestsDTO> suitesData, String envirment, String email) throws IOException {//main List<RunTestsDTO> runData
    	...
    	if (testsList.oldTestss.size() > 0) {
    		String xmlSuiteContent = buildTestsXML(testsList.oldTestss , testsList.oldTestss.get(0).getSuite().getSuite_name());
    		jenkinsURL = runJenkinsSelectedTests(xmlSuiteContent, suite.getSuite_name(), null, envirment, email);
		}
    	if (testsList.newTestss.size() > 0) {
			...
			for (Tests test : testsList.newTestss) {
				if (i == 0) {
					BuildTests = "@" + test.getTest_name();
				}else{
					BuildTests = BuildTests + ',' + "@" +test.getTest_name();
				}
				i++;
			}		
			runTest(findSuite.getJenkins_url() + "buildWithParameters?token=RUN&ManualTests=" + BuildTests
					+ "&Environment="+ envirment + "&Email=" + email);///build?token=RUN
			jenkinsURL = findSuite.getJenkins_url();
		}
		return jenkinsURL;
	}
    ...
    public TestsList buildTestsXML(List<RunTestsDTO> suitesData) throws IOException {
		...
		for (RunTestsDTO runTestsDTO : suitesData) {
			Tests test = testsRepository.findOne(Long.valueOf(runTestsDTO.getId()).longValue());
