 ...
 */
app.controller('SuitesController', function ($scope, $filter, $modal, $log, suitesService, ngTableParams) {
    ...
    function readSuites() {
        suitesService.readSuites().then(function(allSutiesFactory) {
        	   // success
        	suites = allSutiesFactory;
            $scope.data = allSutiesFactory;
    ...
    $scope.setEditId =  function(pid) {
        $scope.editId = pid;
        $scope.udSuite = suitesService.getSuite(parseInt(pid));
        //var a = suitesService.getSuite(parseInt(pid));
        //angular.copy(a, $scope.udSuite);
    }
    ...
    $scope.updateSuite = function (suiteId, suiteName, xmlFileLocation, jenkinsURL) {
        ...
        suitesService.updateSuite(suiteId, suiteName, xmlFileLocation, jenkinsURL).then(function(resultSize) 
        	{// success
	        	$scope.setEditId(-1);
	        	readSuites();
    ...
    $scope.saveSuites = function () {
        ...
        suitesService.saveSuites(suites).then(function(resultSize) 
         {// success
        	readSuites();
    	 }, function(errResponse) {// fail
    		//alert("Server Error");
    ...
    $scope.deleteSuite = function (id) {
        if (confirm("Confirm Delete?") == true) {
        	suitesService.deleteSuite(id).then(function(resultSize) 
        	 {// success
            	readSuites();
        	 }, function(errResponse) {// fail
        		//alert("Server Error");
    ...
    $scope.open = function (size) {
        var modalInstance = $modal.open({
          templateUrl: 'newSuiteModal.html',
          controller: 'newSuiteModalCtrl',
          size: size,
          resolve: {
            items: function () {
...
app.controller('newSuiteController', function ($scope, suitesService) {
    $scope.suites = [];
    ...
    $scope.insertSuite = function (suiteName, xmlFileLocation, jenkinsURL) {
        //alert(suiteName);
        suitesService.newSuite(suiteName, xmlFileLocation, jenkinsURL).then(function(resultSize) 
         {// success
...
app.controller('newSuiteModalCtrl', function ($scope, $log, suitesService, $modalInstance, items) {
