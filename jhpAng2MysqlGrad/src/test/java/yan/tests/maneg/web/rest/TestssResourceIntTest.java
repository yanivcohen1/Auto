...
public class TestssResourceIntTest {
    ...
    public void setup() {
        ...
        final TestssResource testssResource = new TestssResource(testssRepository, testssSearchRepository, stepSearchRepository);
    ...
    public void getTestss() throws Exception {
        ...
        restTestssMockMvc.perform(get("/api/testsses/{id}", testss.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(testss.getId().intValue()))
