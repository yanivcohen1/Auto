...
public class WebDriverUtils extends AutomationScriptingLanguage{

	private static WebDriver driver = null;
	
	public static void setDriver(WebDriver driver_){
		driver = driver_;
	}
	...
	public void assertFalse(String errorMessage, boolean condition) {
	...
	public String readCurrentURL(){
		return executeScript("return window.location.href");
	}
	/*public boolean navigateToRootElement(){
		//WebDriverUtils webDriverUtils = new WebDriverUtils();
		if (driver == null) {
			return super.navigateToRootElement();
		}else{
			//super.driver= driver;
			return navigateToRootElement_(driver);
		}
	}
	...
	boolean navigateToPage(String url){
		//WebDriverUtils webDriverUtils = new WebDriverUtils();
		if (driver == null) {
			return super.navigateToPage(url);
		}else{
			try {
				driver.get(url);
				return true;
	...
	private boolean navigateToRootElement_(WebDriver driver){
		try {
	...
	public void setBrowserDimensionsNoRefresh(int width, int height) {
        driver.manage().window().setSize(new Dimension(width, height));
    }
	
	public static void terminateChromeDriver() {
		...
		if (os.contains("Windows")) {
			...
			String ret = executeCommand("pkill chromedriver");
			System.out.println(ret);
		}
	}
	
	    public List<WebElement> buildDropdownListByCss(String css) {
	        ...
	        try {
	        	...
	        	Reporter.log("buildDropdownList : No <SELECT> element found with identifier [ " + css + " ]");
	        }
	        return elementList;
	    }
	 ...
	 * @return
	 */
	public boolean waitForElementByCssClick(String css) {//, int timeoutSec
		return waitForElementByCssClick(css, false);
	}
	 ...
	 * @return
	 */
	public boolean waitForElementByCssClick(String css, boolean force) {//, int timeoutSec
		boolean find = false;
		for (int i = 0; i < 3; i++) {
			try {
				assertTrue(navigateToRootElement());
				waitForVisibilityOfElementByCss(css);
				buildListByCSS(css);
				int index = 0;
					for (WebElement webElement : getElementList()) {
						navigateToListElement(index);
						if (webElement.isDisplayed() || force) {
							webElement.click();
							find = true;
							return find;
						}else{
	 ...
	 * @return
	 */
	public boolean waitForElementByXpathClick(String xpath) {//, int timeoutSec
		boolean find = false;
		for (int i = 0; i < 3; i++) {
			try {
				assertTrue(navigateToRootElement());
	 ...
	 * @return
	 */
	public boolean waitForElementByIdClick(String id) {//, int timeoutSec
		boolean find = false;
		for (int i = 0; i < 3; i++) {
			try {
				assertTrue(navigateToRootElement());
				waitForVisibilityOfElementById(id);
				buildListByPartialId(id);
				int index = 0;
					for (WebElement webElement : getElementList()) {
						navigateToListElement(index);
						if (webElement.isDisplayed()) {
	 ...
	 * @return
	 */
	public boolean waitForElementByNameClick(String name) {//, int timeoutSec
		boolean find = false;
		for (int i = 0; i < 3; i++) {
			try {
				assertTrue(navigateToRootElement());
				waitForVisibilityOfElementByName(name);
				buildListByName(name);
				int index = 0;
					for (WebElement webElement : getElementList()) {
						navigateToListElement(index);
						if (webElement.isDisplayed()) {
	 ...
	 * @return
	 */
    public List<WebElement> GetRowCells(WebElement table)
    {
        return table.findElements(By.cssSelector("td"));
    }
	
    public List<String> readNetwork(){
    	LogEntries les = driver.manage().logs().get(LogType.PERFORMANCE);
    	List<String> network = new ArrayList<String>();
		for (LogEntry le : les) {
			//System.out.println(le.getMessage());
			network.add(le.getMessage());
        }
		return network;
    }
}