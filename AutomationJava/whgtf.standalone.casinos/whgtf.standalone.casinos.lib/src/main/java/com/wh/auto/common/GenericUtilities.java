...
public class GenericUtilities {
	...
	public static ArrayList<byte[]> takeScreenShot(String testMethodName, String testClassName, WebDriver driver) {
		...
		if (!GraphicsEnvironment.isHeadless()) {
			...
			if (isJenkinsExecution() || screenshotsDir == null) {
				screenshotsDir = "target/cucumber/cucumber-html-report";//"src/test/resources/screenshots/";
			}
	...
	public static String takeScreenShotPath(String testMethodName, String testClassName, WebDriver driver) {
...
//			}
			screenshotsDir = "target/cucumber/cucumber-html-report";
			String environment = System.getProperty("env");
			...
			try {
				...
				outputFolder = new File(screenshotsDir);
				if (!outputFolder.exists()) {
					outputFolder.mkdirs();
				}
			} catch (Exception e1) {
			...
			if ( driver != null){ //MultiThreadedDriverFactory.getDriversMap().size() > 0) {
				...
				try {
					...
					Screenshot driverScreenshot = new AShot()
							.takeScreenshot(driver);
					byte[] screenShot = saveScreenshot(screenshotFileName, driverScreenshot.getImage(), outputFolder);
					screenShoot = saveScreenshotFile(screenshotFileName, driverScreenshot.getImage(), outputFolder);
					screenshotList.add(screenShot);
				} catch (WebDriverException e1) {
					log4j.debug("Unable to take screenshot!");
				...
				try {
					...
					byte[] screenShot = baos.toByteArray();
					saveScreenshot(screenshotFileName, screenShot, outputFolder);
					screenShoot = saveScreenshotFile(screenshotFileName, screenShot, outputFolder);
					screenshotList.add(screenShot);
				} catch (AWTException | IOException e1) {
					log4j.error("Unable to take screenshot!");
				}
	...
	private static String saveScreenshotFile(String screenshotFileName, byte[] screenShot, File outputFolder) {
	...
	private static String saveScreenshotFile(String screenshotFileName, BufferedImage screenShot, File outputFolder) {
	...
	public static void quitWebDriver(WebDriver next) {
		...
		if (next != null) {
			...
			try {
			if (next instanceof AndroidDriver) {
				((AndroidDriver) next).quit();
			} else if (next instanceof IOSDriver) {
				((IOSDriver) next).quit();
			}else{
				next.close();
				Thread.sleep(1000);
