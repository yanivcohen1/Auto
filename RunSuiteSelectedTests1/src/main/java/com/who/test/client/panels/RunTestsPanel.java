...
import com.who.test.client.panels.manage.ISearchFilter;
...
public class RunTestsPanel extends Composite implements ISearchFilter{
	...
	public final DialogBox dialogBoxWithButten = new DialogBox();
	...
	public RunTestsPanel() {
		...
		targetPanel.setSize("300px", "615px");
	...
	protected void runTests() {
		...
		if (tests.size() == 1) {
			dialogWithButten("Select tests first");
		} else {
			GWT.log(elements);
			SuitesData suitesData = new SuitesData();
	...
	public String executeFilter(String searchWord) {
		if (!searchWord.trim().isEmpty()) {
			...
			for (String testName : this.testsNames) {
				if (testName.contains(searchWord)) {
