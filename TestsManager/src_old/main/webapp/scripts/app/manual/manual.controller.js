...
angular.module('testmanagerApp').controller('ManualController',['$scope', '$log', '$window', '$http', '$q', '$resource', 'uiGridConstants', '$location', 
                                                                    function ($scope,   $log,   $window,   $http,   $q,   $resource,   uiGridConstants,   $location) {
    ...
    $scope.showContent = function($fileContent){
        ...
        contentJson = JSON.parse(CSV2JSON($scope.content));
        $scope.gridOptions.data = JSON.parse(CSV2JSON($scope.content));
    };
