...
class RecentlyPlayedGamesAPI extends Simulation {
  ...
  lazy val recentlyPlayedGameController = conf.getString("recentlyPlayedGameController")
  ...
  val recentlyPlayedGamesScenario = recentlyPlayedGameController + "?username=test_cj56789"

  //set GET params
  val scn = scenario("Recently played games API performance")
    .exec(http(recentlyPlayedGamesScenario)
      .get(recentlyPlayedGamesScenario)
      .check(regex("gameCode")))
