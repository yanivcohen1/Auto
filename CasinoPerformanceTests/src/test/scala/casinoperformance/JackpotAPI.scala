...
class JackpotAPI extends Simulation {
  ...
  lazy val jackpotController = conf.getString("jackpotController")
  ...
  val currencyGBPscenario = jackpotController + "?info=2&casino=playtech&currency=gbp"
  val currencyUSDscenario = jackpotController + "?info=2&casino=playtech&currency=usd"
  val currencyEURscenario = jackpotController + "?info=2&casino=playtech&currency=eur"

  //set GET params
  val scn = scenario("Jackpot API performance")
    .exec(http(currencyGBPscenario)
      .get(currencyGBPscenario))
    .exec(http(currencyUSDscenario)
      .get(currencyUSDscenario))
    .exec(http(currencyEURscenario)
      .get(currencyEURscenario)
      .check(regex("info")))
