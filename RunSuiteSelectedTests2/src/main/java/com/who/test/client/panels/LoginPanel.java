...
public class LoginPanel extends Composite {
	...
	private Widget managePanel;
	
	public LoginPanel(Widget _managePanel) {
		this.managePanel = _managePanel;
		verticalPanel = new VerticalPanel();
		initWidget(verticalPanel);
		verticalPanel.setSize("304px", "");
		...
		lblLoginToYour.setStyleName("sendButton");
		...
		btnSignIn.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {submitForm(); }
		});
	...
	protected void submitForm() {
		...
		if (textBoxUsername.getText().length() == 0
			...
			UserData.setPassword(textBoxPassword.getText());
				...
				public void onSuccess(String UserType) {
					if (UserType != null) {
						if (UserType.contentEquals("Admin")) {
							...
							verticalPanel.add(managePanel);*/
							((MenuPanel)managePanel).loadPanel();
