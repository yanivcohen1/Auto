...
public class RegisterPage extends WebDriverUtils{
	...
	public RegisterPage() {
		if (!init) {
			init = true;
			if(AppContextRepository.getAc().containsBean(RegisterPage.class.getName()))
				thisClass = (RegisterPage) AppContextRepository.getAc().getBean(RegisterPage.class.getName());
			else
				Reporter.log("Propertys "+RegisterPage.class.getName()+" error");
	...
	public void Click_Register_top_Butten() throws Throwable {
		SharedSteps.getScenario().write("Start Register");
		Thread.sleep(5000);
		assertTrue(navigateToRootElement());
	...
	@Given("^Desktop User Click Join Now Butten$")
	public void user_Click_Join_Now_Butten() throws Throwable {
		Thread.sleep(5000);
		assertTrue(navigateToRootElement());
		String css = "#playButton";
		assertTrue(waitForVisibilityOfElementByCss(css));
		assertTrue(navigateToElementByCSS(css));
		assertTrue(click());
		Thread.sleep(2000);
		navigateToFrameByXpath("//iframe[contains(@src,'http://www.williamhillcasino.com/en/registration')]");
	}
	...
	public void Contiue_to_step(int arg1) throws Throwable {
		SharedSteps.getScenario().write("Start Step2");
		assertTrue(navigateToRootElement());
	...
	@Given("^User Deposit limit \"(.*?)\" limit Amount \"(.*?)\"$")
	...
	public void _EG_Account_was_Created_(String msg) throws Throwable {
		...
		Thread.sleep(3000);*/
		assertTrue(navigateToRootElement());
		String xpath = thisClass.getDepositlink();
		
			boolean find = waitForVisibilityOfElementByXpath(xpath);//20 sec
			if(!find){
				find = waitForVisibilityOfElementByXpath(xpath);//20 sec
			}
			if (find) {
				navigateToElementByXpath(xpath);
				click();
				//waitForVisibilityOfElementByXpath("//div[@class='" + xphat + "']");
				//waitForVisibilityOfElementByCss(css);
				//waitForElementToBeClickableByCss(css);
			}else{
				if (SharedSteps.getScenario().getName().contentEquals("RegisterWhcc_en") || 
					SharedSteps.getScenario().getName().contentEquals("RegisterEg_en")) {
					String console = ChromeConsoleLogging.GetConsolLog(SharedData.getBrowsers().get_Driver());
					ElasticSearch.elasticSearchAddResults("Portal3", "RegistrationEndMsg", false, console);
		...
		/*try {
			assertTrue(navigateToRootElement());
			String css = thisClass.getDepositlink();
			//Thread.sleep(5000);
			waitForElementToBeClickableByCss(css);
			click();
			//waitForElementByCssClick(css);
		} catch (Exception e) {	}*/
		//assertTrue(navigateToElementByCSS(css));
		//assertTrue(click());
